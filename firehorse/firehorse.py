...
def main():
    ...
    parser = argparse.ArgumentParser(\
        prog="firehorse",
        usage="firehorse -t TARGET_NAME [options] cmd [cmd_args]",
        ...
        epilog=textwrap.dedent('''\
                                ...
                                Usage examples:
                                    firehorse -s -c COM17 --fh-loader "C:\Program Files (x86)\Qualcomm\QPST437\bin\fh_loader.exe" --sahara-server "C:\Program Files (x86)\Qualcomm\QPST437\bin\QSaharaServer.exe" -t nokia6 target magic
                                    firehorse -c COM17 --fh-loader "C:\Program Files (x86)\Qualcomm\QPST437\bin\fh_loader.exe" --sahara-server "C:\Program Files (x86)\Qualcomm\QPST437\bin\QSaharaServer.exe" -t nokia6 fw hello
                                    firehorse -c COM17 --fh-loader "C:\Program Files (x86)\Qualcomm\QPST437\bin\fh_loader.exe" --sahara-server "C:\Program Files (x86)\Qualcomm\QPST437\bin\QSaharaServer.exe" -t nokia6 fw peek 0x100000 0x10
                                '''))
    parser.add_argument('-c', '--com', dest='com', help='Specify COM port', required=True)
    ...
    parser.add_argument('--fh-loader', dest='fh_loader', help='Path to fh_loader.exe', required=True)
    parser.add_argument('--sahara-server', dest='sahara_server', help='Path to QSaharaServer.exe', required=True)
    ...
    constants.COM = args.com
    constants.FH_LOADER = args.fh_loader
    constants.SAHARA_SERVER = args.sahara_server
